...
#   #folder_id       = data.terraform_remote_state.organization.outputs.shared_sub_folders.fldr-shared-vpc.id
#   folder_id       = 856237993723
...
module "logging_monitoring_project" {
  ...
  folder_id       = data.terraform_remote_state.bootstrap1.outputs.shared_sub_folders.fldr-common.name
  labels          = local.project_terraform_labels
}
...
module "cicd_project" {
  source          = "../modules/projects"
  name            = "prj-${local.environment}-${local.cicd_project_label}"
  project_id      = "prj-${local.environment}-${local.cicd_project_label}"
  services        = local.cicd_apis
  billing_account = var.billing_account
  #folder_id       = data.terraform_remote_state.organization.outputs.folders.fldr-shared.name
  folder_id       = data.terraform_remote_state.bootstrap1.outputs.shared_sub_folders.fldr-common.name
  #folder_id       = var.folder_id
...
module "artifact_project" {
  source          = "../modules/projects"
  name            = "prj-${local.environment}-${local.artifact_project_label}"
  project_id      = "prj-${local.environment}-${local.artifact_project_label}"
  services        = local.artifact_apis
  billing_account = var.billing_account
  #folder_id       = data.terraform_remote_state.organization.outputs.folders.fldr-shared.name
  folder_id       = data.terraform_remote_state.bootstrap1.outputs.shared_sub_folders.fldr-common.name
  #folder_id       = var.folder_id
